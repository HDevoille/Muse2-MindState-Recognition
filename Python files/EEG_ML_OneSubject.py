...
Focus = pd.concat([F01,F02,F03,F04,F05,F06],ignore_index=True)
...
Relax = pd.concat([R01,R02,R03,R04,R05,R06],ignore_index=True)
...
X = Global_Data[['Delta','Theta','Alpha','Beta','Gamma','F1','F3','F4']]
y = Global_Data[['label']]
...
scores = cross_validate(randf,X_train,y_train.values.ravel(),cv=cv,return_estimator=True)
sorted(scores.keys())
...
X_new = F_test[['Delta','Theta','Alpha','Beta','Gamma','F1','F3','F4']]
...
X_new2 = R_test[['Delta','Theta','Alpha','Beta','Gamma','F1','F3','F4']]
